...
new_data = {'relation':[], 'no_matcheadas':[], 'contradiction':[],
            ...
            'TextoL':[],'HipotesisL':[],'pInflexion':[],'pInflexionV':[],'pInflexionV2':[],'entSimilitud':[],'dicEntT':[],'dicEntH':[],
...
#for i in range(4):
    ...
    if(type(hipotesis[i])!=type(1.0)):
        l_t=ut.get_lemmas_(textos[i],nlp)
        l_h=ut.get_lemmas_(hipotesis[i],nlp)
        s1=l_t
        s2=l_h
        #BoW
    ...
    t_lem_temp=ut.get_lemmasALL_(textos[i],nlp)
    h_lem_temp=ut.get_lemmasALL_(hipotesis[i],nlp)
    t_lem=[]
    h_lem=[]
    for a in t_lem_temp:
        if a not in t_lem:
    ...
    new_data['pInflexionV'].append(utils.entropia(ma.max().round(1).values.flatten()))
    ...
    new_data['list_m'].append(ma.shape[1])
    new_data['pInflexionV2'].append(utils.entropia(ma.max().round(1).values.flatten()))
